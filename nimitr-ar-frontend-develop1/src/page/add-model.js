    ...
    return(
        <Container maxWidth="lg" sx={{ mt:3 , }}>
            {loading ? <SkeletonManageProjectPage /> : <>
                ...
                qr={projectData?.projectId?.imageurl} 
                />
                {previewMarkerStatus?.status && <PreviewMarker 
                url={previewMarkerStatus?.markerUrl} 
                name={previewMarkerStatus?.markerName} 
                onClosePrviewMarkerDialog={handleClosePreviewMarker} 
                status={previewMarkerStatus?.status} />}
                {previewMediaStatus?.status && 
                <PreviewMedia url={previewMediaStatus?.mediaUrl} 
                name={previewMediaStatus?.mediaName} 
                onClosePrviewMediaDialog={handleClosePreviewMedia} 
                status={previewMediaStatus?.status} />}
                {previewQrCodeStatus && <PreviewQR onClosePrviewQrDialog={handleTogglePreviewQr} 
                url={projectData?.projectId?.imageurl} 
                status={previewQrCodeStatus} />}
            </>}
        </Container>
    );
};